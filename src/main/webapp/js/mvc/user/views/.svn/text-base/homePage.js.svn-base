...
var mask = $('#maskMethodVal option:selected').val();
            ...
            for (var p=0;p<output.length;p++) {
          		var newDiv = "";
          		 if(m == 0){
          	      ...
          	      divName = "usCon"+ output[p].resId;          	    
          	      newDiv += '<div class="item '+active+'" id='+divName+' style="width: 960px;"><a href="javascript:zoomOut();" class="zoom'+output[p].resId+'"> Zoom Back'+output[p].resId+'</a></div>'; 
          	      divName = "#" +divName;
          	      $(".carousel-control").removeAttr("style");
          	      $('#map-inner-block').append(newDiv);
 ...
 if($('#dsTheme').val()=="")
      ...
      else if(!(/^www\.+[A-Za-z0-9.\/%&=\?_:;-]+$/.test($('#dsPath').val()))){
     ...
     //alert("wrapper: "+JSON.stringify(dataObj.wrapper));
      ...
      if(dataObj.initParam.timeWindow=="")dataObj.initParam.timeWindow=300;
