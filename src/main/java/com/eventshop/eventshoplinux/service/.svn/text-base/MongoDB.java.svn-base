...
//		result = (JsonObject) JSON.parse(s);	// ERROR: com.mongodb.BasicDBObject cannot be cast to com.google.gson.JsonObject
//		return result;
//	}
//	
	...
	public static void main(String[] args){
		...
		dateQuery.put("date", new BasicDBObject("$gt", "2013-08-05").append("$lt", "2013-08-10"));
//		BasicDBObject latQuery = new BasicDBObject();
//		latQuery.put("latitude", new BasicDBObject("$gt", 24).append("$lt", 40));
//		BasicDBObject lngQuery = new BasicDBObject();
//		lngQuery.put("longitude", new BasicDBObject("$gt", -125).append("$lt", -66));
//		
		

		Double radious = 10.0/3959.0;
		//BasicDBObject geoQuery = (BasicDBObject) JSON.parse("{'loc':{$geoWithin:{$centerSphere:[[-120.680278,38.201944],"+radious+"]}}}");
		BasicDBObject geoQuery = (BasicDBObject) JSON.parse("{'loc':{$geoWithin:{$centerSphere:[[-120.6,38.2],"+radious+"]}}}");
		...
		String qstr = "{$and:[{date:{$gt:'2013-08-05',$lt:'2013-08-10'}},{'loc':{$geoWithin:{$centerSphere:[[-120.6,38.2],"+radious+"]}}}]}";
		//BasicDBObject query = (BasicDBObject) JSON.parse(qstr);
		String fStr = "loc,date,value";
		List<DBObject>  result = myDB.find(qstr, fStr);
		
		for(DBObject r: result){
			BasicDBList loc = ((BasicDBList)((DBObject)r.get("loc")).get("coordinates"));
			System.out.println(r.get("date") + "," + r.get("value") + "," + loc.get(0)+","+loc.get(1));
		}
	}
}
