package com.eventshop.eventshoplinux.util.datasourceUtil.simulator;
...
import com.eventshop.eventshoplinux.domain.datasource.simulator.DistributionGenerator;
import com.eventshop.eventshoplinux.domain.datasource.simulator.GaussianGenerator2D;
...
//	Thread producerThread = null;
	...
	public Simulator(SpatialPoint sw, SpatialPoint ne, double latUnit, double longUnit, long timeWindow,
			String theme, Kernel kernel)
	{
		...
		rows = (int)Math.ceil((BigDecimal.valueOf(nePoint.lat)).subtract(BigDecimal.valueOf(swPoint.lat), context).divide(BigDecimal.valueOf(latUnit), context).doubleValue());
		cols = (int)Math.ceil((BigDecimal.valueOf(nePoint.lon)).subtract(BigDecimal.valueOf(swPoint.lon), context).divide(BigDecimal.valueOf(longUnit), context).doubleValue());
	...
	public void setParameters(ArrayList<DistParameters> params) {
		if(k == Kernel.gaussian) 
		{
			for(DistParameters param: params)
				generators.add(new GaussianGenerator2D((GaussianParameters2D) param));
		}
		
		int i = 0, j = 0;
		for(double lat = swPoint.lat; lat < nePoint.lat && i < rows; lat = lat + latUnit)
		{
			for(double lon = swPoint.lon; lon < nePoint.lon && j < cols; lon = lon + longUnit)
			{
				double value = 0;
		...
		producer = new DataProducer(swPoint, nePoint, latUnit, longUnit, timeWindow, theme, count, rows, cols, q);
	}
	...
	public static void main(String[] args)
	{
		...
		Simulator sim = new Simulator(swPoint, nePoint, latUnit, longUnit, 30*1000, "flu", k);
		
		// Generate the parameters
		ArrayList<DistParameters> params = new ArrayList<DistParameters>();
		
		// LA
		GaussianParameters2D gParam = new GaussianParameters2D(34.1, -118.2, 3.0, 3.0, 50);
		params.add(gParam);
		// SF
