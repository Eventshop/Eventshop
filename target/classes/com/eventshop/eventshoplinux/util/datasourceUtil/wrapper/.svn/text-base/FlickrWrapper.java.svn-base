...
public class FlickrWrapper extends Wrapper
{
    ...
    public void run()
    {
    	...
    	d.setTime(d.getTime() - params.timeWindow); //timewindow unit should be seconds	
    	searchParams.setMinTakenDate(d); 
    	
    	// 3. Loop for Lat Long blocks 
    	for(double i = params.swLat; i < params.neLat; i = i+params.latUnit) 
    	{
    		for(double j = params.swLong; j < params.neLong && isRunning; j = j+params.longUnit) 
    		{
    			if(!isRunning) break;
    			
    			log.info("<BR>" + i +" , " + j +"<BR>");
    			 
    			searchParams.setBBox(Double.toString(j),Double.toString(i), Double.toString(j+params.longUnit),Double.toString(i+params.latUnit));
    	        ...
    	        if(photoList != null) {
    	        	...
    	        	point = new STTPoint(photoList.size(), d, startTime.getTime(), params.latUnit, params.longUnit, i, j, theme);
    	        	ls.add(point);
    	        	for(int k = 0; k < photoList.size(); k++){
    	        		// get photo object
    	        		Photo photo = (Photo)photoList.get(k);
    	        		log.info("<a href="+photo.getUrl()+"><img border=\"0\" src=\""+photo.getSmallUrl()+"\"></a>");          
    	           }
    	        }	
    		}
    	}
    ...
    public static void main(String[] args) {
        try {
        	...
        	wrapper.setBagOfWords(new String[]{"Allergy","Flu", "Dogs", "Dog", "Nature"});
        	wrapper.run();
        	
        	while (wrapper.hasNext()) {
        		STTPoint point = wrapper.next();
        		wrapper.log.info(point.value + " , ");
        	}
        } catch(Exception e) {
        	log.error(e.getMessage());
        }
