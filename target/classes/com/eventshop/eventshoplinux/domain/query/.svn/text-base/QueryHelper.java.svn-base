...
public class QueryHelper {
	...
	/*	QueryListDAO queryListDAO = null; 
		for (int i = 0; i < queryDTOList.size(); ++i) {
			queryListDAO = new QueryListDAO();
			boolean idExist = queryListDAO.chkQryID(Integer.parseInt(queryDTOList.get(i).getqID()));
			if (!idExist) {
				// new query gets added to DB
		...
		if (statFLag.equals(RUN)) {
			...
			for (int i = 0; i < queryDTOList.size(); ++i) {
				...
				output = (query.contextPath+query.outputName + ".csv");
			...
			output = (query.contextPath+query.outputName + ".csv"); //throws null pointer for now -- sanjukta
	...
	/*public DataSource parseDataSource(String srcText)
	{
		srcText = srcText.replace("\\", "\\\\");
        JSONObject outerObject = JSONObject.fromObject(srcText);
        JSONObject srcObject = outerObject.getJSONObject("source");
        JSONObject visualObject = srcObject.getJSONObject("visualParam");
        JSONObject initParamObject = srcObject.getJSONObject("initRes");
        JSONObject finalParamObject = srcObject.getJSONObject("finalRes");
        
        DataSource dataSrc = new DataSource();
        dataSrc.srcID = outerObject.getLong("srcID");
        dataSrc.srcTheme = srcObject.getString("theme");
        dataSrc.srcName = srcObject.getString("name");
        dataSrc.url = srcObject.getString("url");
        ...
        if(dataSrc.srcFormat == DataFormat.stream)
        {
        	dataSrc.supportedWrapper = srcObject.getString("supportedWrapper");
        	try {
        		dataSrc.bagOfWords = (ArrayList<String>) JSONArray.toCollection(srcObject.getJSONArray("bagOfWords"), Class.forName("java.util.ArrayList"));
        	} catch (ClassNotFoundException e) {
        		log.error(e.getMessage());
        	}
        }
        
        if(!visualObject.isNullObject())
        {
        	dataSrc.createVisualObject();
        	String tranPath = visualObject.getString("tranMatPath");
        	dataSrc.visualParam.tranMatPath = (tranPath == null? tempDir + "tranMat.txt" : tranPath);
        	
        	String colorPath = visualObject.getString("colorMatPath");
        	dataSrc.visualParam.colorMatPath = (colorPath == null? tempDir + "colorMat.txt" : colorPath);
        	
        	String maskPath = visualObject.getString("maskPath");
        	// note: maskpath is optional, so it can be null
        	if(maskPath == null)
        		dataSrc.visualParam.maskPath = tempDir + "mask.png";
        	else if(maskPath.equalsIgnoreCase(""))
        		dataSrc.visualParam.maskPath = null;
        	else
        		dataSrc.visualParam.maskPath = maskPath;
        	
        	
        	String ignoreNumber = visualObject.getString("ignoreSinceNumber");
        	
        	dataSrc.visualParam.ignoreSinceNumber = ((ignoreNumber == null || ignoreNumber.equalsIgnoreCase(""))? -1: visualObject.getInt("ignoreSinceNumber"));
        	
        	log.info("maskPath |" + maskPath + "|" + ignoreNumber + dataSrc.visualParam.maskPath);
        	
        }
        
        if(!initParamObject.isNullObject())
        {
        	dataSrc.createInitParam();
        	dataSrc.initParam.timeWindow = initParamObject.getLong("timeWindow")*1000;
	        dataSrc.initParam.syncAtMilSec = initParamObject.getLong("syncAtMilSec")*1000;
	        dataSrc.initParam.latUnit = initParamObject.getDouble("latUnit");
	        dataSrc.initParam.longUnit = initParamObject.getDouble("longUnit");
	        dataSrc.initParam.swLat = initParamObject.getDouble("swLat");
	        dataSrc.initParam.swLong = initParamObject.getDouble("swLong");
	        dataSrc.initParam.neLat = initParamObject.getDouble("neLat");
	        dataSrc.initParam.neLong = initParamObject.getDouble("neLong");
	        dataSrc.initParam.numOfRows = (int) Math.abs(Math.ceil((dataSrc.initParam.neLat 
	        		- dataSrc.initParam.swLat) / dataSrc.initParam.latUnit));
	        dataSrc.initParam.numOfColumns = (int) Math.abs(Math.ceil((dataSrc.initParam.neLong 
	        		- dataSrc.initParam.swLong) / dataSrc.initParam.longUnit));
        }
        
        if(!finalParamObject.isNullObject())
        {
        	dataSrc.createFinalParam();
        	dataSrc.finalParam.timeWindow = finalParamObject.getLong("timeWindow")*1000;
	        dataSrc.finalParam.syncAtMilSec = finalParamObject.getLong("syncAtMilSec")*1000;
	        dataSrc.finalParam.latUnit = finalParamObject.getDouble("latUnit");
	        dataSrc.finalParam.longUnit = finalParamObject.getDouble("longUnit");
	        dataSrc.finalParam.swLat = finalParamObject.getDouble("swLat");
	        dataSrc.finalParam.swLong = finalParamObject.getDouble("swLong");
	        dataSrc.finalParam.neLat = finalParamObject.getDouble("neLat");
	        dataSrc.finalParam.neLong = finalParamObject.getDouble("neLong");
	        dataSrc.finalParam.numOfRows = (int) Math.abs(Math.ceil((dataSrc.finalParam.neLat 
	        		- dataSrc.finalParam.swLat) / dataSrc.finalParam.latUnit));
	        dataSrc.finalParam.numOfColumns = (int) Math.abs(Math.ceil((dataSrc.finalParam.neLong 
	...
	public FrameParameters getFinalParamFromSrc(int qryId) {
		...
		/*if(index < sources.size())
			return sources.get(index).finalParam;*/
	...
	private String parseFilter(QueryDTO qryDTO)	{
		...
		// if 1 datasource add that otherwise look for the array of datasources and add them all in a loop
		...
		/*if(dataSrc.indexOf(DSR_DS) != -1)
		{
			...
			for(int i = 0; i < newQuery.queriedSources.size(); ++i)
			{
				...
				if(srcID == newQuery.queriedSources.get(i).srcID)
				{
					srcFound = true;
		...
		{
			//double[] coords = (double[]) JSONArray.toArray(content.getJSONArray("coords"), double.class);
		...
		// generate the mask
		String maskVar = "";
		if(qryDTO.getMaskMethod().equals(MAP) || qryDTO.getMaskMethod().equals(TEXTUAL))
		{
			maskVar = "mask" + maskID;
	...
	private String parseGrouping(QueryDTO qryDTO)
	{
		...
		if(doColoring.equals("true"))
		{
			...
			{
				colorVector += ("\tgrpColors.push_back(" + colors[i].trim().toLowerCase() + ");\n");
			}
		}
		...
		else if(method.equals("Thresholds"))
		{
			...
			{
				groupCC += ("\tthresh" + grpCritID + ".push_back(" + thresholds[i].trim() + ");\n");
			}
	...
	private String parseAggregation(QueryDTO qryDTO)
	{
		...
		for(int i = 0; i < srcArray.length; i++)
		{
			...
			/*{
				...
				{
					// If the data source has been added once, then ignore it
					if(srcID == newQuery.queriedSources.get(j).srcID)
					{
						srcFound = true;
						// Change the data Src to be the dsID_subscript
						dataSrc = dataSrc + "_" + srcSubscript;
						...
						// update the srcVarName
						DataSource src = new DataSource(newQuery.queriedSources.get(j));
						src.srcVarName = dataSrc;
						newQuery.queriedSources.add(src);
						break;
					}
		...
		if(valueArray != null && valueArray.length > 0)
		{
			...
			// get aggOperator
			String operation = qryDTO.getAggOperator().substring(3).toUpperCase();
			// get normalization
			if(!qryDTO.getValueNorm().equals("valueNorm") || qryDTO.getValueNorm().equals("false"))
			{
	...
	private String parseSpatialMatching(QueryDTO qryDTO)
	{
		...
		int src = qryDTO.getPatternSrc(); //patternsrc int to do 
		...
		else if(src == 1)
		{
			...
			spMatchingCC += "\tSpatialPatternMatching " + opVar + "(" + dataSrc + ", " + patternType
	...
	private String parseTemporalChar(QueryDTO qryDTO)
	{
		...
		String tpcharCC;
		tpcharCC = "\tTemporalChar " + opVar + "(" + dataSrc + ", " + tpcharOp + ", " + window + "*1000);\n\n";
		return tpcharCC;	
	}
	...
	private String parseTemporalMatching(QueryDTO qryDTO)
	{
		...
		String valueNorm = qryDTO.getValueNorm().toLowerCase();
		String patternSrc = Integer.toString(qryDTO.getPatternSrc());//patternSrc should be string to do
